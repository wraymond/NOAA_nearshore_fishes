...
## Spatial ##
NO_SEAK <- readOGR(dsn="Data", layer="NO_SEAK")
NI_SEAK <- readOGR(dsn="Data", layer="NI_SEAK")
SO_SEAK <- readOGR(dsn="Data", layer="SO_SEAK")
SI_SEAK <- readOGR(dsn="Data", layer="SI_SEAK")
```
...
```{r}
event <- unique(fish.event[, c(2, 11, 12)])
write.csv(event, "Data/NOAA_SEAK_EventID.csv", row.names = FALSE)

xy <- event[, c(2,3)]
event.sp <- SpatialPointsDataFrame(coords = xy, data = event, proj4string = CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))

event.sp <- spTransform(event.sp, CRS = "+proj=utm +zone=8 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs")
```


```{r region assign}
test <- point.in.poly(event.sp, NO_SEAK)
